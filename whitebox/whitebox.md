...
所有模型使用同一 1154 篇 benchmark、30-step HotFlip、`delta >= 0.1`
成功阈值和 10% token 编辑上限。D_HOTFLIP 按固定 256 篇分层子集选择为
`gstep600`，而不是 clean QWK 最高的 `best/`。

| 模型 | Clean QWK | MAE | ASRΔ@0.10 | Mean Δ | Upward band ASR |
|------|----------:|----:|----------:|-------:|----------------:|
| B0 | 0.8377 | 0.3923 | 96.10% | 0.1314 | 12.82% |
| C0 | 0.8350 | 0.4098 | 98.61% | 0.1454 | 11.87% |
| D_HOTFLIP `gstep600` | 0.8216 | 0.4050 | 78.08% | 0.1143 | 12.39% |

D_HOTFLIP 相对 C0：

- Clean QWK 下降 0.0134，满足不超过 0.02 的效用门槛。
- ASR 下降 20.53 个百分点，满足至少下降 15 个百分点的防御门槛。
- Mean Δ 从 0.1454 降到 0.1143。
...
长时间运行的训练、clean 评估和逐篇攻击默认显示进度条与 ETA；需要纯文本日志时
可在评估命令中增加 `--no-progress`。
