...
RESULT_FILE = os.path.join(OUT_DIR, "hotflip_valid_1154_result.json")
PROGRESS_FILE = os.path.join(OUT_DIR, "hotflip_valid_1154_progress.json")
...
for idx, (text, _) in enumerate(essays):
    ...
    if (idx + 1) % 50 == 0:
        ...
        with open("/tmp/hotflip_valid_1154_progress.json", "w") as f:
            json.dump(progress, f)
        shutil.copy("/tmp/hotflip_valid_1154_progress.json", PROGRESS_FILE)
...
with open("/tmp/hotflip_valid_1154_result.json", "w") as f:
    json.dump(result, f)
shutil.copy("/tmp/hotflip_valid_1154_result.json", RESULT_FILE)
