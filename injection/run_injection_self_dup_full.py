#!/usr/bin/env python3
"""
Self-Duplication Injection Attack — FULL 1134 essays (统一搜索策略)

核心：复制 essay 已有句子插入，语义连贯难以被检测。
...
RESULT_FILE = os.path.join(OUT_DIR, "injection_self_dup_full_result.json")
PROGRESS_FILE = os.path.join(OUT_DIR, "injection_self_dup_full_progress.json")
...
def build_candidates(text: str):
    """
    Self-duplication: 复制 essay 已有句子插入到其他位置。
    4 source × 4 dest = 16 候选
    """
...
if __name__ == "__main__":
    ...
    for idx, (text, _) in enumerate(essays):
        ...
        if (idx + 1) % 50 == 0:
            ...
            with open("/tmp/injection_self_dup_full_progress.json", "w") as f:
                json.dump(progress, f)
            shutil.copy("/tmp/injection_self_dup_full_progress.json", PROGRESS_FILE)

    asr = n_ok / N_ESSAYS
    elapsed = time.time() - t0
    print(f"\n=== Self-Duplication FULL ===", flush=True)
    ...
    result = {
        ...
        "params": {"n_steps": N_STEPS, "beam_size": 1, "max_candidates": MAX_CANDIDATES,
                   "threshold": THRESHOLD, "attack_type": "self_duplication"},
        "details": details
    }
    with open("/tmp/injection_self_dup_full_progress.json", "w") as f:
        json.dump(result, f)
    shutil.copy("/tmp/injection_self_dup_full_progress.json", RESULT_FILE)
