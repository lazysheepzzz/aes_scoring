#!/usr/bin/env python3
"""
External Wikipedia Injection Attack — 30 篇测试

核心：插入 Wikipedia 预制句子到任意位置，对齐统一搜索策略。
...
RESULT_FILE = os.path.join(OUT_DIR, "injection_wiki_30_result.json")
PROGRESS_FILE = os.path.join(OUT_DIR, "injection_wiki_30_progress.json")
...
def build_candidates(text: str, sentence_bank: list):
    """
    External Wikipedia injection: 插入 Wikipedia 句子到任意位置。

    候选构建：
      - 位置：所有句子边界（0 到 n），随机选 4 个
      - 内容：从 Wikipedia 句子库随机选 4 句
      - 16 候选 = 4 位置 × 4 句子
    """
    ...
    # 所有可插入的位置（插在 sentences[i] 之前，i 从 0 到 n）
    dest_positions = list(range(n + 1))

    candidates = []
    seen = set()

    # 随机选 4 个 dest 位置，4 个 Wikipedia 句子
...
def iterative_injection(scorer, text: str, sentence_bank: list):
    """
    Wikipedia injection with scorer-guided greedy search.
...
if __name__ == "__main__":
    ...
    for idx, (text, _) in enumerate(essays):
        ...
        if (idx + 1) % 10 == 0:
            ...
            with open("/tmp/injection_wiki_30_progress.json", "w") as f:
                json.dump(progress, f)
            shutil.copy("/tmp/injection_wiki_30_progress.json", PROGRESS_FILE)

    asr = n_ok / N_ESSAYS
    elapsed = time.time() - t0
    print(f"\n=== Wikipedia External Injection ===", flush=True)
    ...
    with open("/tmp/injection_wiki_30_progress.json", "w") as f:
        json.dump(result, f)
    shutil.copy("/tmp/injection_wiki_30_progress.json", RESULT_FILE)
